 ...
 */
export function initializeAgentAPI(cfg) {
    config = cfg;
}
/**
...
function authenticateAgent(req, res, next) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        res.status(401).json(errorResponse('Missing or invalid authorization header', 'UNAUTHORIZED', 401));
        return;
    }
 ...
 */
router.post('/get-split-instructions', authenticateAgent, async (req, res) => {
    try {
        const { agentWallet, amount, referralCode, paymentTxSignature } = req.body;
        // Validation
        if (!agentWallet || !amount || !paymentTxSignature) {
            res.status(400).json(errorResponse('Missing required fields: agentWallet, amount, paymentTxSignature', 'MISSING_FIELDS', 400));
            return;
        }
        console.log('');
        console.log('═══════════════════════════════════════════════');
        ...
        if (!merchant) {
            ...
            res.status(404).json(errorResponse('Merchant not found for agent wallet', 'MERCHANT_NOT_FOUND', 404));
            return;
        }
        ...
        res.status(500).json(errorResponse(error instanceof Error ? error.message : 'Failed to get split instructions', 'SPLIT_INSTRUCTIONS_ERROR', 500));
    }
});
/**
 ...
 */
router.post('/confirm-split', authenticateAgent, async (req, res) => {
    try {
        const { splitId, settlementTx, status, agentWallet, referralCode } = req.body;
        if (!splitId || !settlementTx || !status) {
            res.status(400).json(errorResponse('Missing required fields: splitId, settlementTx, status', 'MISSING_FIELDS', 400));
            return;
        }
        console.log('');
        console.log('═══════════════════════════════════════════════');
        ...
        res.status(500).json(errorResponse(error instanceof Error ? error.message : 'Failed to confirm split', 'SPLIT_CONFIRMATION_ERROR', 500));
    }
});
/**
