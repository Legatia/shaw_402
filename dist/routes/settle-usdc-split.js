...
router.post('/', async (req, res) => {
    try {
        ...
        if (!agentPrivateKey || !usdcMint || !recipients || !Array.isArray(recipients)) {
            res.status(400).json(errorResponse('Missing required fields: agentPrivateKey, usdcMint, recipients', 'INVALID_INPUT', 400));
            return;
        }
        if (recipients.length === 0 || recipients.length > 10) {
            res.status(400).json(errorResponse('Recipients array must contain 1-10 recipients', 'INVALID_RECIPIENTS', 400));
            return;
        }
        ...
        console.log('');
        // Parse recipients
        const parsedRecipients = recipients.map((r, index) => {
        ...
        res.status(500).json(errorResponse(error.message || 'Failed to settle USDC split', 'SETTLEMENT_ERROR', 500));
    }
});
export default router;
//# sourceMappingURL=settle-usdc-split.js.map