...
export class NonceDatabase {
     ...
     */
    async markNonceUsed(nonce, transactionSignature) {
        return new Promise((resolve, reject) => {
            const sql = `
        UPDATE nonces 
     ...
     */
    async updateTransactionSignature(nonce, transactionSignature) {
        return new Promise((resolve, reject) => {
            const sql = `
        UPDATE nonces 
    ...
    async getNonceStats() {
      ...
      `;
            const currentTime = Date.now();
            this.db.get(sql, [currentTime, currentTime], (err, row) => {
                if (err) {
     ...
     */
    async updateTransactionStatus(transactionSignature, status, errorMessage = null) {
        return new Promise((resolve, reject) => {
            const sql = `
        UPDATE transactions 
...
//# sourceMappingURL=nonce-database.js.map