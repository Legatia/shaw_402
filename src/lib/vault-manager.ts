...
export interface VaultConfig {
  ...
  rewardShareRate: number; // Percentage of staking rewards shared with merchants (e.g., 0.8 = 80%)
}
...
export class VaultManager {
   ...
   */
  async getVaultUSDCAccount(): Promise<PublicKey> {
    return await getAssociatedTokenAddress(
      this.config.usdcMint,
      this.config.vaultKeypair.publicKey
    );
  }
   ...
   */
  async getVaultSOLBalance(): Promise<bigint> {
   ...
   */
  async getVaultUSDCBalance(): Promise<bigint> {
    try {
   ...
   */
  async getTVL(): Promise<{ SOL: string; USDC: string; totalUSD: string }> {
   ...
   */
  getMinimumDeposits(): { SOL: string; USDC: string } {
    return {
