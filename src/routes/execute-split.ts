...
export interface ExecuteSplitRequest {
  ...
  usdcMint: string;
  agentPrivateKey: string; // Agent's private key (base58) - secured by x402 auth
}
 ...
 */
export function executeSplitRoute(context: ExecuteSplitContext) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      const { splitId, recipients, agentUSDCAccount, usdcMint, agentPrivateKey } = req.body as ExecuteSplitRequest;
      ...
      if (!splitId) {
        res.json(
          errorResponse('Split ID is required', 'MISSING_SPLIT_ID', 400)
        );
        return;
      }
      ...
      if (!recipients || !Array.isArray(recipients) || recipients.length === 0) {
        res.json(
          errorResponse('Recipients array is required', 'MISSING_RECIPIENTS', 400)
        );
        return;
      }
      ...
      if (!agentUSDCAccount) {
        res.json(
          errorResponse('Agent USDC account is required', 'MISSING_AGENT_ACCOUNT', 400)
        );
        return;
      }
      ...
      if (!usdcMint) {
        res.json(
          errorResponse('USDC mint address is required', 'MISSING_USDC_MINT', 400)
        );
        return;
      }
      ...
      if (!agentPrivateKey) {
        res.json(
          errorResponse('Agent private key is required', 'MISSING_PRIVATE_KEY', 400)
        );
        return;
      }
      ...
      for (const recipient of recipients) {
        if (!recipient.wallet || !recipient.amount || !recipient.usdcAccount) {
          res.json(
            errorResponse(
              `Invalid recipient: missing wallet, amount, or usdcAccount`,
              'INVALID_RECIPIENT',
              400
            )
          );
        ...
        try {
          ...
          res.json(
            errorResponse(
              `Invalid public key in recipient: ${recipient.wallet} or ${recipient.usdcAccount}`,
              'INVALID_PUBLIC_KEY',
              400
            )
          );
      ...
      try {
        ...
        if (agentKeypair.publicKey.toString() !== agentWallet) {
          res.json(
            errorResponse(
              'Agent private key does not match x402 authorized wallet',
              'KEYPAIR_MISMATCH',
              403
            )
          );
        ...
        res.json(
          errorResponse(
            'Invalid agent private key format',
            'INVALID_PRIVATE_KEY',
            400
          )
        );
      ...
      const signature = await context.solanaUtils.splitUSDCPayment(
        agentKeypair,
      ...
      res.status(500).json(
        errorResponse(
          error instanceof Error ? error.message : 'Split execution failed',
          'SPLIT_EXECUTION_FAILED',
          500
        )
      );
    }
