...
let config: VaultAPIConfig;

/**
 * Initialize vault API routes
 */
export function initializeVaultAPI(cfg: VaultAPIConfig): void {
  config = cfg;
}
...
router.get('/info', async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json(
      errorResponse(
        error instanceof Error ? error.message : 'Failed to get vault info',
        'VAULT_INFO_ERROR',
        500
      )
    );
 ...
 */
router.post('/verify-deposit', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!merchantId || !txSignature || !amount || !token) {
      res.status(400).json(
        errorResponse(
          'Missing required fields: merchantId, txSignature, amount, token',
          'MISSING_FIELDS',
          400
        )
      );
    ...
    const depositToken = token as 'SOL' | 'USDC';
    if (!config.vaultManager.validateDepositAmount(depositAmount, depositToken)) {
      ...
      res.status(400).json(
        errorResponse(
          `Deposit amount below minimum requirement of ${minimums[depositToken]} ${token}`,
    ...
    const verified = await config.vaultManager.verifyDeposit(
      txSignature,
      depositAmount,
    ...
    if (!verified) {
      res.status(400).json(
        errorResponse('Deposit verification failed', 'VERIFICATION_FAILED', 400)
      );
      return;
    }
    ...
    res.status(500).json(
      errorResponse(
        error instanceof Error ? error.message : 'Failed to verify deposit',
...
router.post('/withdraw', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!merchantId || !depositId) {
      res.status(400).json(
        errorResponse('Missing required fields: merchantId, depositId', 'MISSING_FIELDS', 400)
      );
      return;
    }
    ...
    if (deposit.merchantId !== merchantId) {
      res.status(403).json(
        errorResponse('Deposit does not belong to this merchant', 'UNAUTHORIZED', 403)
      );
      return;
    }
    ...
    if (deposit.status !== 'active') {
      res.status(400).json(
        errorResponse(`Deposit is not active (status: ${deposit.status})`, 'INVALID_STATUS', 400)
      );
      return;
    }
    ...
    // Execute withdrawal
    const result = await config.vaultManager.returnDeposit(
      merchant.merchantWallet,
      totalAmount,
    ...
    if (!result.success) {
      res.status(500).json(
        errorResponse(result.error || 'Withdrawal failed', 'WITHDRAWAL_FAILED', 500)
      );
      return;
    }

    // Update deposit status
    await config.affiliateDb.updateMerchantDepositStatus(
      depositId,
      'withdrawn',
      Date.now(),
      result.txSignature
 ...
 */
router.get('/deposit/:depositId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (deposit.status === 'active' && deposit.depositToken === 'SOL') {
      ...
      const calculatedRewards = config.vaultManager.calculateRewards(
        BigInt(deposit.depositAmount),
    ...
    res.status(500).json(
      errorResponse(
        error instanceof Error ? error.message : 'Failed to get deposit info',
        'DEPOSIT_INFO_ERROR',
        500
      )
    );
 ...
 */
router.get('/merchant/:merchantId/deposits', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json(
      errorResponse(
        error instanceof Error ? error.message : 'Failed to get merchant deposits',
        'MERCHANT_DEPOSITS_ERROR',
        500
      )
    );
  }
