...
router.post('/:merchantId/cancel', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!merchantWallet || !signature || !message) {
      res.status(400).json(
        errorResponse('Missing required fields: merchantWallet, signature, message', 'INVALID_INPUT', 400)
      );
      return;
    }
    ...
    if (error.message?.includes('insufficient')) {
      res.status(500).json(
        errorResponse(
          'Platform wallet does not have enough SOL for refund. Please contact support.',
