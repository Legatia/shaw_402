...
let config: SolanaPayConfig;

/**
 * Initialize Solana Pay routes with configuration
 */
export function initializeSolanaPayRoutes(cfg: SolanaPayConfig) {
  config = cfg;
}
...
type PaymentEndpoint = keyof typeof PAYMENT_ENDPOINTS;
...
router.get('/:endpoint', async (req: Request, res: Response) => {
  try {
    ...
    if (!isValidEndpoint(endpoint)) {
      res.status(404).json(
        errorResponse('Payment endpoint not found', 'INVALID_ENDPOINT', 404)
      );
      return;
    }
...
router.post('/:endpoint', async (req: Request, res: Response) => {
  try {
    ...
    if (!account) {
      res.status(400).json(
        errorResponse('Missing account in request body', 'MISSING_ACCOUNT', 400)
      );
      return;
    }
    ...
    const paymentInfo = PAYMENT_ENDPOINTS[endpoint as PaymentEndpoint];
 ...
 */
router.get('/:endpoint/qr', async (req: Request, res: Response) => {
  try {
    const { endpoint } = req.params;
 ...
 */
router.get('/:endpoint/status/:reference', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/:endpoint/validate', async (req: Request, res: Response) => {
  try {
    const { endpoint } = req.params;
    ...
    if (!reference) {
      res.status(400).json(
        errorResponse('Missing reference in request body', 'MISSING_REFERENCE', 400)
      );
      return;
    }
    ...
    try {
      await validateTransfer(
        ...
        signatureInfo.signature,
        {
          ...
          amount: BigInt(paymentInfo.amount) as any, // Type compatibility with @solana/pay BigNumber
      ...
      if (error instanceof ValidateTransferError) {
        res.status(400).json(
          errorResponse('Payment validation failed', 'INVALID_PAYMENT', 400)
        );
      } else {
        throw error;
      }
    ...
    if (error instanceof FindReferenceError) {
      res.status(404).json(
        errorResponse('Transaction not found', 'TRANSACTION_NOT_FOUND', 404)
      );
    } else {
 ...
 */
router.get('/transfer/:endpoint', async (req: Request, res: Response) => {
  try {
    const { endpoint } = req.params;
    ...
    const url = encodeURL({
      recipient: config.merchantAddress,
      amount: amountInSol as any, // Type compatibility with @solana/pay BigNumber
      reference,
    ...
    res.status(500).json(
      errorResponse(
        error instanceof Error ? error.message : 'Transfer URL generation failed',
        'TRANSFER_URL_ERROR',
        500
      )
    );
 ...
 */
function isValidEndpoint(endpoint: string): endpoint is PaymentEndpoint {
